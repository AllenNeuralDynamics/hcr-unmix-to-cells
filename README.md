...
**Classes** (`gmm_classes: true`, on in `p3_mixed_inhibitory_gmm_no_gfp`) label *every* cell. A
Slc17a7 GMM threshold is fitted on all cells (same routine as the gate); the first matching rule wins:

| `class` | Rule |
|---|---|
| `Inhibitory` | in the GMM inhibitory set (after the Slc17a7 cutoff), even when Slc17a7+ |
| `Excitatory` | Slc17a7+ |
| `Unassigned` | neither (not `None`, which pandas reads back as missing) |

`slc17a7_positive` keeps the Slc17a7 call for every cell. `inhibitory_gmm/classes/` holds the
Slc17a7 threshold and QC plot; `inhibitory_gmm/cell_classes.csv` holds every cell's labels and
spot counts.
...
a given cell are left blank for that row. When the inhibitory GMM ran, `kmean_cluster_<k>`
